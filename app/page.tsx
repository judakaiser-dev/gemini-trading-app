...
function Input({ label, id, value, onChange, darkMode = true }: { label: string; id: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; darkMode?: boolean }) {
  return (
    <div className="w-full">
      <label htmlFor={id} className={`block text-sm font-medium ${darkMode ? 'text-gray-400' : 'text-gray-700'} mb-1.5`}>{label}</label>
      <input 
        id={id} 
        value={value} 
        onChange={onChange} 
        className={`px-3.5 py-2.5 rounded-xl transition-all ${darkMode ? 'bg-white/8 border border-white/15 text-white placeholder:text-gray-500 focus:bg-white/12 focus:border-white/25' : 'bg-white border border-black/8 text-black placeholder:text-gray-400 focus:bg-white focus:border-black/15'} text-sm w-full focus:outline-none`} 
      />
    </div>
  );
}
